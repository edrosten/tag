...
/// a simpler algorithm for sigma points. For expensive functions the difference in evaluated points may matter.

namespace Internal {

template <class F, int N, class V, class M> void outer_product_upper_half(const TooN::FixedVector<N,V>& v, const double w, TooN::FixedMatrix<N,N,M>& m)
{
    for (int i=0; i<N; ++i)
        for (int j=i; j<N; ++j)
...
template <class F, int N, class V1, class V2, class M> void outer_product_upper_half(const TooN::FixedVector<N,V1>& v1, const TooN::FixedVector<N,V2>& v2, const double w, TooN::FixedMatrix<N,N,M>& m)
{
    for (int i=0; i<N; ++i)
        for (int j=i; j<N; ++j)
